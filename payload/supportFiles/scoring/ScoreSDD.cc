...
void ScoreSDD::ClearOutputFiles() {
	...
	//--Added by Nicolas--
	//This file will contain info on every lesion
...
G4bool ScoreSDD::ProcessHits(G4Step* aStep,G4TouchableHistory*)
{
	...
	if (fBuildNucleus) {
		...
		//Nicolas checking stuff out
		//G4cout << voxIDZ << "," << voxIDX << " ,"  << voxIDY << " ,"  << fNumVoxelsPerSide  << " ,"  << fVoxelID << G4endl;
	}
...
void ScoreSDD::OutputEvent(G4int ModeOfAction, G4int PositionOnBp, G4ThreeVector position, G4int voxelID, G4int index){
	...
	// Record data
	outFile << ModeOfAction  << fDelimiter << position[0] << fDelimiter << position[1]  << fDelimiter << position[2] 
...
void ScoreSDD::OutputSDDHeader()
{
	...
	outFile << "Dose or fluence, variable " << fDoseThreshold/gray << ";\n";
	...
	outFile << "Time " << "0" << ";\n"; //done
	outFile << "Damage and primary count, variable, variable "<< block_pairs.size() << ", " << fNumEvents << ";\n";
