                                            ...
                                            G4bool isVisu)
{
    ...
    {
        G4cerr<<"FatalError: GeoVolumeV2::BuildLogicFiber. A class parameter has not been initialized and its value is still negative"<<G4endl;
...
std::map<G4String, std::vector<G4LogicalVolume*> >* GeoVolumeV2::CreateNucleosomeCuttedSolidsAndLogicals(std::vector<DNAPlacementData>* nucleosomeVolumePositions,
                                                                                                       std::map<G4ThreeVector, G4double>* posAndRadiusMap,
                                                                                                       G4bool isVisu)
{
    ...
    {
        G4cerr<<"************************************************************"<<G4endl;
        G4cerr<<"GeoVolumeV2::CreateNucleosomeCuttedSolidsAndLogicals: fSugarTHFRadius, fSugarTMPRadius or fBaseRadius were not set. Fatal error."<<G4endl;
        G4cerr<<"************************************************************"<<G4endl;
        std::exit(EXIT_FAILURE);
    }
